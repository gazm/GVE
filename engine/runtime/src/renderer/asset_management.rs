...
use super::test_geometry::create_test_sdf;
...
impl Renderer {
        ...
        match load_geometry_from_binary(
            ...
            Ok(result) => {
                ...
                if self.loaded_volumes.contains_key(&asset_id) {
                     ...
                     log::info!("✅ Asset {}: Defaulting to SDF view (splat overlay: {})", asset_id, has_splats);
    ...
    pub fn set_active_sdf(&mut self, asset_id: Option<u64>) {
        ...
        // ViewMode determines what combination of data is used
    }
